...
namespace SimulaRV.ViewModels
{
    ...
    class ShuttlePageViewModel : Screen, IHandle<bool>
    {
        ...
        public async Task WriteVariableAsync()
        {
            ...
            {
                await Global.ErrorAsync(_windowManager, "Controller selezionato non valido");
                return;
            }
            ...
            {
                var retValAsync = await ((SimulaShuttle_Ctr)AppViewModel.Instance.SelectedController).WriteVariableTrasloAsync(VariableTraslo, VariableValue);
                if (retValAsync != "OKWD") { retVal = 1; }
            }

            if (retVal == 1)
            {
                await Global.ErrorAsync(_windowManager, "Errore di scrittura variabile. Verifica la connessione e riprova");
                return;
            }
