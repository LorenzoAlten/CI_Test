    ...
    class HandlingPageViewModel : Screen, IHandle<bool>
    {
        ...
        protected async Task InitAsync()
        {
            ...
            await Task.Run(() =>
            {
                ...
                var missions = MisMission.GetList<MisMission>(
                    Global.Instance.ConnGlobal,
                    "WHERE (MIS_L1_MST_Code = 'WSHP' AND MIS_L2_MST_Code = 'WSHP') OR (MIS_L1_MST_Code = 'EXEH' AND MIS_L2_MST_Code = 'EXEH')");
                if (missions.Count > 0)
                {
                    var mission = missions.First();
                    TelegramType = ETelegramTypes.LCAP;
                    Position = mission.MIS_Current_MOD_Code;
                    MissionID = mission.MIS_Id;
            ...
            }).ContinueWith(antecedent =>
            {
                Application.Current.Dispatcher.Invoke(() =>
                {
                    ...
                    TelegramType = (TelegramType <= ETelegramTypes.ACKT) ? ETelegramTypes.DREQ : TelegramType;
                });
        ...
        private void OnTelegramReceived(object sender, GenericEventArgs e)
        {
            ...
            else if (telegram.TelegramType == ETelegramTypes.CMND)
            {
                TelegramType = ETelegramTypes.CTRL;
                Position = telegram.Position;
                ClaWeight = 1000;
