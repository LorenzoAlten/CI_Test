...
using static mSwAgilogDll.Errevi.Machine_Tel;
...
namespace SimulaRV.ViewModels
{
    ...
    class AgvPageViewModel : Screen, IHandle<bool>
    {
        ...
        public async Task WriteVariableAsync()
        {
            if (VariableValue < 0)
            {
                Global.ErrorAsync(_windowManager, "Valore variabile non valido");
                return;
            }
            ...
            if (retVal != "OKWD")
            {
                Global.ErrorAsync(_windowManager, "Errore di scrittura variabile. Verifica la connessione e riprova");
                return;
            }
