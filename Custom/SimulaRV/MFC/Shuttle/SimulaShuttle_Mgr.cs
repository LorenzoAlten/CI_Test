...
namespace SimulaRV
{
    public class SimulaShuttle_Mgr : TrafficMgrBase
    {
        #region Members
        ...
        public SimulaShuttle_Mgr(SqlConnection connection, DataRow row)
            : base(connection, row)
        {
        ...
        protected override bool Init()
        {
            ...
            {
                var controller = c as SimulaShuttle_Ctr;
                if (controller != null)
        ...
        protected void PING()
        {
            foreach (SimulaShuttle_Ctr controller in _controllers)
            {
                SimulaShuttle_Tel telegram = new SimulaShuttle_Tel(ETelegramTypes.PING, controller.Code, "WCS");
